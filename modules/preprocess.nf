process PREPROCESS {
  ...
  output:
  tuple val(meta), path("${meta.id}.preprocessed.h5ad")
  ...
  """
  preprocess.py --input ${input} --id ${meta.id} ${transfer} ${no_symbols} ${min_counts} ${max_counts} ${min_genes} ${max_genes} ${max_pct_mito} --output ${meta.id}.preprocessed.h5ad
  """
}